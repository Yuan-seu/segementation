...
def engine(series, a, filename, errornumber):
    ...
    #     if judge_st:  # 如果st满足再进行线性拟合
    #         for key in keys:
    #             error, k, b, judge_linear = measure(seg_list[key], errornumber)
    ...
    #             # 如果当前数据满足条件则更新数据， 如果不满足则沿用上一个的数据
    #             store_win_info[key] = {'number': windows_number, 'error': error, 'k': k, 'b': b,
    #                                    'window_start': window_start, 'file': filename, 'len': window_len}
    #         window_len += 1  # 扩大窗口
    #         window_end += 1
    #
    #     else:  # 如果下一个数据不满足条件则并指拓展并初始化参数
    #         """ 根据上述的判断条件，决定是否在当前节点后进行分段，如果分段，将当前分段的信息存储在Windows中，并进行下一个窗口的划动"""
    #         for key in keys:
    #             windows[key].append(store_win_info[key])  # 把对应的变量的分段信息存储进来--> {'IA':[], 'IB':[]..}
    #             seg_list[key] = []
...
if __name__ == '__main__':
    ...
    for err in tqdm(range(0, 20, 0.1)):
        windows = engine(series, a, filename, err, )  # 返回窗口信息
        x1, x2 = parameter.input(windows)
        winerror.append(x1)
        winnum.append(x2)
        mongo.input(windows, database, err)
        show.input(filename, database, err)
    #
    filename = 'error.txt'
    with open(filename, 'w') as file_object:
        for i in range(len(winnum)):
            file_object.write(str(winerror[i])+'\n')
    filename = 'number.txt'
    with open(filename, 'w') as file_object:
        for i in range(len(winnum)):
            file_object.write(str(winnum[i]) + '\n')

    # print(least_square.cal(np.array(y),np.array(x)))
    # print(winnum)
    # print(winerror)
    # plt.rcParams['figure.figsize'] = (40, 24)  # 设置figure_size尺寸
    # plt.plot(winnum, color="orange", label="Fitting Line", linewidth=1)
    # plt.plot(winerror, color="black", label="Fitting Line", linewidth=1)
    # plt.savefig('1.jpg')
    # plt.show()
